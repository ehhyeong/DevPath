...
public class NodeClearanceService {
  ...
  public List<NodeClearanceResponse.Detail> recalculate(
      Long userId, NodeClearanceRequest.Recalculate request) {
    validateUser(userId);
    roadmapRepository
        .findByRoadmapIdAndIsDeletedFalse(request.getRoadmapId())
        .orElseThrow(() -> new CustomException(ErrorCode.ROADMAP_NOT_FOUND));

    List<RoadmapNode> nodes =
        roadmapNodeRepository.findAllByRoadmapRoadmapId(request.getRoadmapId()).stream()
            .sorted(
                Comparator.comparing(
                        RoadmapNode::getSortOrder, Comparator.nullsLast(Integer::compareTo))
  ...
  public NodeClearanceResponse.Detail getNodeClearance(Long userId, Long nodeId) {
    return nodeClearanceRepository
  ...
  public List<NodeClearanceResponse.Detail> getNodeClearances(Long userId, Long roadmapId) {
    if (roadmapId == null) {
      return nodeClearanceRepository.findAllByUserIdOrderByLastCalculatedAtDesc(userId).stream()
          .map(this::toDetail)
          .toList();
    }

    roadmapRepository
        .findByRoadmapIdAndIsDeletedFalse(roadmapId)
        .orElseThrow(() -> new CustomException(ErrorCode.ROADMAP_NOT_FOUND));
    ...
    if (!clearances.isEmpty()) {
      return clearances.stream().map(this::toDetail).toList();
    }
  ...
  @Transactional(readOnly = true)
  public List<NodeClearanceResponse.ReasonDetail> getNodeClearanceReasons(
      Long userId, Long nodeId) {
    NodeClearance clearance = ensureNodeClearance(userId, nodeId);

    return nodeClearanceReasonRepository
  ...
  public NodeClearanceResponse.ProofCheck proofCheck(Long userId, Long nodeId) {
    NodeClearance clearance = ensureNodeClearance(userId, nodeId);
    List<NodeClearanceResponse.ReasonDetail> reasons =
        nodeClearanceReasonRepository
            .findAllByNodeClearanceIdOrderByIdAsc(clearance.getId())
            .stream()
            .map(this::toReasonDetail)
            .toList();
  ...
  public NodeClearanceResponse.Detail synchronizeNodeClearance(Long userId, Long nodeId) {
    ...
    NodeClearanceEvaluator.EvaluationResult evaluationResult =
        nodeClearanceEvaluator.evaluate(userId, nodeId);

    NodeClearance nodeClearance =
        nodeClearanceRepository
            .findByUserIdAndNodeNodeId(userId, nodeId)
            .orElseGet(() -> NodeClearance.builder().user(user).node(node).build());
  ...
  private NodeClearanceResponse.ReasonDetail toReasonDetail(
      NodeClearanceReason nodeClearanceReason) {
