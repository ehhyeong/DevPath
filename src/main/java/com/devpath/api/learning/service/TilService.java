...
import com.devpath.api.learning.provider.BlogPublishProvider;
import com.devpath.api.learning.provider.BlogPublishResult;
...
public class TilService {
  ...
  public TilResponse getTil(Long userId, Long tilId) {
    TilDraft til =
        tilDraftRepository
            .findByIdAndUserIdAndIsDeletedFalse(tilId, userId)
  ...
  public List<TilResponse> getTilList(Long userId) {
    return tilDraftRepository.findByUserIdAndIsDeletedFalseOrderByCreatedAtDesc(userId).stream()
  ...
  @Transactional
  public TilPublishResponse publishToExternalBlog(
      Long userId, Long tilId, TilPublishRequest request) {
    TilDraft til =
        tilDraftRepository
            .findByIdAndUserIdAndIsDeletedFalse(tilId, userId)
            .orElseThrow(() -> new CustomException(ErrorCode.TIL_NOT_FOUND));
  ...
  private BlogPublishProvider findBlogPublishProvider(String normalizedPlatform) {
    return blogPublishProviders.stream()
        ...
        .findFirst()
        .orElseThrow(
            () -> new CustomException(ErrorCode.INTERNAL_SERVER_ERROR, "지원하지 않는 블로그 플랫폼입니다."));
  }
  ...
  private String buildTableOfContents(String content) {
    ...
    while (matcher.find()) {
      ...
      String anchor =
          title.toLowerCase().replaceAll("[^a-z0-9가-힣\\s]", "").trim().replaceAll("\\s+", "-");

      entries.add(
          String.format("{\"level\":%d,\"title\":\"%s\",\"anchor\":\"%s\"}", level, title, anchor));
    }
