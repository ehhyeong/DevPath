...
public class InstructorMentoringBoardService {
  ...
  private final InstructorMentoringWorkspaceSynchronizer workspaceSynchronizer;
  private final InstructorMentoringBoardLiveDataAssembler liveDataAssembler;
  ...
  public InstructorMentoringBoardService(
    ...
    this.liveDataAssembler = liveDataAssembler;
  ...
  public InstructorMentoringBoardPayload getBoard(Long instructorId) {
    ...
    if (board.isEmpty()) {
      return liveDataAssembler.attachLiveData(instructorId, createDefaultPayload(instructorId));
    }

    InstructorMentoringBoardPayload payload =
        liveDataAssembler.filterPersistedLiveData(
            workspaceSynchronizer.synchronize(
    ...
    return liveDataAssembler.attachLiveData(instructorId, payload);
  }
  ...
  public InstructorMentoringBoardPayload saveBoard(
    ...
    payload =
        liveDataAssembler.filterPersistedLiveData(
            workspaceSynchronizer.synchronize(instructorId, payload));
    String payloadJson = writePayload(payload);
  ...
  private InstructorMentoringBoardPayload createDefaultPayload(Long instructorId) {
    List<InstructorMentoringBoardPayload.OngoingProjectItem> ongoingProjects =
        workspaceSynchronizer.getDefaultOngoingProjects(instructorId);
