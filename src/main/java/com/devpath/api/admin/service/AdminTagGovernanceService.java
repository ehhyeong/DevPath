...
public class AdminTagGovernanceService {
  ...
  public TagGuideResponse getTagGuide() {
    List<TagResponse> standardTags =
        tagRepository.findAll().stream()
            .filter(
                tag ->
                    Boolean.TRUE.equals(tag.getIsOfficial())
                        && !Boolean.TRUE.equals(tag.getIsDeleted()))
            .sorted(Comparator.comparing(Tag::getName))
            .map(TagResponse::from)
            .collect(Collectors.toList());

    String guideMessage =
        "1. 모든 태그는 소문자 kebab-case를 권장합니다.\n"
            + "2. 중복 의미 태그는 관리자 병합 기능으로 정리합니다.\n"
            + "3. category는 backend, frontend, infra처럼 일관된 분류를 사용합니다.";

    return TagGuideResponse.builder().standardTags(standardTags).guideMessage(guideMessage).build();
  }

  private void moveCourseTagMappings(Tag sourceTag, Tag targetTag) {
    List<CourseTagMap> sourceMappings =
        courseTagMapRepository.findAllByTagTagId(sourceTag.getTagId());

    for (CourseTagMap sourceMapping : sourceMappings) {
      Long courseId = sourceMapping.getCourse().getCourseId();

      if (!courseTagMapRepository.existsByCourseCourseIdAndTagTagId(
          courseId, targetTag.getTagId())) {
  ...
  private void moveUserTechStacks(Tag sourceTag, Tag targetTag) {
    List<UserTechStack> sourceTechStacks =
        userTechStackRepository.findAllByTagTagId(sourceTag.getTagId());
