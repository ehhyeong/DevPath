...
public class AdminPermissionController {
  ...
  public ApiResponse<RoleResponse> updateRole(
      @PathVariable Long roleId, @RequestBody @Valid RoleCreateRequest request) {
    return ApiResponse.success(
        "Role이 수정되었습니다.", adminPermissionService.updateRole(roleId, request));
  }
  ...
  public ApiResponse<List<String>> getPermissionCodes() {
    return ApiResponse.success(
        "사용 가능한 관리자 권한 코드를 조회했습니다.", AdminAuthorityService.supportedAuthorities());
  }

  @DeleteMapping("/roles/{roleId}")
  public ApiResponse<Void> deleteRole(@PathVariable Long roleId) {
    adminPermissionService.deleteRole(roleId);
    return ApiResponse.success("관리자 Role을 삭제했습니다.", null);
  }
  ...
  public ApiResponse<UserPermissionResponse> assignAdminRole(
      @PathVariable Long userId, @RequestBody @Valid AdminRoleAssignmentRequest request) {
    return ApiResponse.success(
        "관리자 Role을 배정했습니다.", adminPermissionService.assignAdminRole(userId, request));
  }

  @DeleteMapping("/users/{userId}/admin-role")
  public ApiResponse<UserPermissionResponse> clearAdminRole(@PathVariable Long userId) {
    return ApiResponse.success(
        "관리자 Role 배정을 해제했습니다.", adminPermissionService.clearAdminRole(userId));
  }

  @Operation(summary = "강사 등급 변경", description = "강사 등급을 변경합니다.")
  @PatchMapping("/users/{userId}/role")
  public ApiResponse<Void> changeInstructorGrade(
      @PathVariable Long userId, @RequestBody @Valid InstructorGradeUpdateRequest request) {
