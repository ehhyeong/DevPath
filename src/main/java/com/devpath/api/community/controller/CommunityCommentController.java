...
@Tag(
    ...
    description =
        "커뮤니티 댓글/대댓글 API입니다. Swagger 테스트 기준으로 userId=1은 게시글 작성자, userId=2는 댓글 작성자로 사용하면 됩니다.")
public class CommunityCommentController {
  ...
  public ApiResponse<CommentResponse> createReply(
    ...
    CommentResponse response =
        communityCommentService.createReply(userId, postId, commentId, request);
    return ApiResponse.ok(response);
  }
  ...
  public ApiResponse<List<CommentResponse>> getComments(
      @Parameter(description = "댓글을 조회할 게시글 ID입니다.", example = "10") @PathVariable Long postId) {
