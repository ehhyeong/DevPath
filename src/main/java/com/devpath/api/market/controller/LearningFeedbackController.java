...
public class LearningFeedbackController {
  ...
  public ResponseEntity<ApiResponse<LearningFeedbackResponse.AddToRoadmapResult>> addToRoadmap(
    ...
    return ResponseEntity.ok(ApiResponse.ok(learningFeedbackService.addToRoadmap(userId, request)));
  }
  ...
  public ResponseEntity<ApiResponse<LearningFeedbackResponse.RecommendedCourses>>
      getRecommendedCourses(
          @Parameter(hidden = true) @AuthenticationPrincipal Long userId,
          @Valid @RequestBody LearningFeedbackRequest.Courses request) {
    return ResponseEntity.ok(
        ApiResponse.ok(learningFeedbackService.getRecommendedCourses(userId, request)));
  }
}
