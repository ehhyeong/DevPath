...
public interface DiagnosisQuizRepository extends JpaRepository<DiagnosisQuiz, Long> {
     ...
     */
    boolean existsByUser_UserIdAndRoadmap_RoadmapId(Long userId, Long roadmapId);

    /**
     * 특정 사용자가 특정 로드맵에 대해 수행한 가장 최근 진단 퀴즈 조회
     */
    @Query("SELECT dq FROM DiagnosisQuiz dq " +
            "WHERE dq.user.userId = :userId AND dq.roadmap.roadmapId = :roadmapId " +
     ...
     */
    Optional<DiagnosisQuiz> findByQuizIdAndUser_UserId(Long quizId, Long userId);
}
