...
class SwaggerSmokeIntegrationTest {
  ...
  void openApiSpecIncludesRecentCourseEndpoints() throws Exception {
    mockMvc
        ...
        .andExpect(jsonPath("$.paths['/api/courses'].get.summary").isNotEmpty())
        .andExpect(
            jsonPath("$.paths['/api/me/roadmaps/{roadmapId}/recommendations/init'].post.summary")
                .isNotEmpty())
        .andExpect(jsonPath("$.paths['/api/me/skills/check'].post.summary").isNotEmpty())
        .andExpect(jsonPath("$.tags.length()").value(greaterThanOrEqualTo(3)));
  }
  ...
  void swaggerUiConfigExposesGroupedApiDocs() throws Exception {
    mockMvc
        ...
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.urls.length()").value(greaterThanOrEqualTo(3)))
        .andExpect(jsonPath("$.urls[*].name").value(hasItem("강사")));
  }
  ...
  void seededCourseApisRespondWithSampleData() throws Exception {
    mockMvc
        ...
        .andExpect(
            jsonPath("$.data.instructor.channelApiPath")
                .value("/api/instructors/" + instructorId + "/channel"))
